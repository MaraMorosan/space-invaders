export const CFG = {
  playerDamage: 2,
  playerLivesMax: 5,
  playerLivesStart: 5,
  autoFireMs: 180,
  waveEveryMs: 3000,
  waveBurstDelay: 220,
  waveCountMin: 3,
  waveCountMax: 5,
  bossCountdownStart: 25,
  gutterX: 150,
  bossSpawnY: 60,
  bossEntryInvulnMs: 1200,
  bossHitCooldownMs: 180,
  bossMinHits: 20,
  bossMinHitsGrowth: 6,

  bossHpMultiplierBase: 0.7,
  bossHpGrowthPerKill: 0.2,
  bossHpCapMultiplier: 3.0,
  bossFireDelayFloor: 140,
  bossBulletSpeedBase: 200,
  bossBulletSpeedPerKill: 20,
  bossBulletSpeedCapKills: 12,
  enemyBulletPenaltyBase: 40,
  enemyBulletPenaltyPerKill: 4,
};

export type EnemySpec = {
  key: string;
  hp: number;
  speed: [number, number];
  fxTint: number;
  score: number;
  scale?: number;
};

export const ENEMY_TYPES: EnemySpec[] = [
  { key: 'enemy_small', hp: 1, speed: [90, 130], fxTint: 0x00e676, score: 10 },
  { key: 'enemy_fast', hp: 2, speed: [100, 140], fxTint: 0xff7043, score: 20 },
  { key: 'enemy_tank', hp: 3, speed: [70, 110], fxTint: 0xffd54f, score: 30 },
];

export type BossSpec = {
  key: string;
  hp: number;
  fxTint?: number;
  scale: number;
  speedX: number;
  speedY: number;
  fireDelay: number;
  fireWindowMs: number;
  firePauseMs: number;
};

export const BOSSES: BossSpec[] = [
  {
    key: 'boss_beetle',
    hp: 50,
    fxTint: 0xffc107,
    scale: 2.0,
    speedX: 110,
    speedY: 24,
    fireDelay: 320,
    fireWindowMs: 1100,
    firePauseMs: 900,
  },
  {
    key: 'boss_manta',
    hp: 70,
    fxTint: 0x00e5ff,
    scale: 2.2,
    speedX: 90,
    speedY: 20,
    fireDelay: 260,
    fireWindowMs: 900,
    firePauseMs: 1100,
  },
  {
    key: 'boss_brute',
    hp: 90,
    fxTint: 0x1e88e5,
    scale: 2.3,
    speedX: 130,
    speedY: 18,
    fireDelay: 360,
    fireWindowMs: 1300,
    firePauseMs: 1000,
  },
];
